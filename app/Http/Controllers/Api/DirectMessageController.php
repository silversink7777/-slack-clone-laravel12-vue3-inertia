...
use App\Models\DirectMessage; // Added this import for the new method

class DirectMessageController extends Controller
{
    protected $directMessageRepository;

    public function __construct(DirectMessageRepositoryInterface $directMessageRepository)
    {
    ...
    {
        try {
            $partners = $this->directMessageRepository->getPartners(auth()->id());
            
            return response()->json($partners);
        } catch (\Exception $e) {
            Log::error('Failed to get direct message partners: ' . $e->getMessage());
    ...
    public function getConversation(Request $request): JsonResponse
    {
        try {
            ...
            $messages = $this->directMessageRepository->getConversation(
                auth()->id(),
                (int) $partnerId,
                50
            );
            ...
            // メッセージを既読にする
            $this->directMessageRepository->markConversationAsRead(auth()->id(), (int) $partnerId);
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            $message = $this->directMessageRepository->create([
    ...
    {
        try {
            Log::info('Direct message update request received', ['message_id' => $id, 'data' => $request->all()]);
            
            $message = $this->directMessageRepository->findById($id);
            ...
            // 自分のメッセージのみ編集可能
            if (!$this->directMessageRepository->isUserMessage($id, auth()->id())) {
                return response()->json([
                    'error' => 'Unauthorized',
            ...
            $this->directMessageRepository->update($message, [
                'content' => $validated['content'],
            ]);

            $message->load(['sender', 'receiver']);

            Log::info('Direct message updated successfully', ['message_id' => $message->id]);
            ...
            Log::error('Direct message update failed: ' . $e->getMessage());
            
            return response()->json([
                'error' => 'Direct message update failed',
                'message' => $e->getMessage()
            ], 500);
        }
    }
    ...
    public function destroy(string $id): JsonResponse
    {
        try {
            ...
            $this->directMessageRepository->delete($message);
            return response()->json(['success' => true]);
        } catch (\Exception $e) {
            return response()->json([
                'error' => 'Direct message delete failed',
                'message' => $e->getMessage()
            ], 500);
        }
    }
    ...
    {
        try {
            $count = $this->directMessageRepository->getUnreadCount(auth()->id());
    ...
    {
        try {
            $success = $this->directMessageRepository->markAsRead($id, auth()->id());
