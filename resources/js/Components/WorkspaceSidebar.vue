...
let unreadDMCountInterval = null;
...
const fetchUnreadDMCount = async () => {
    try {
        ...
        if (response.data && typeof response.data.count === 'number') {
            unreadDMCount.value = response.data.count;
        }
    } catch (error) {
        console.error('Failed to fetch unread DM count:', error);
...
// 定期的に未読DM数を更新
const startUnreadDMCountInterval = () => {
    if (unreadDMCountInterval) clearInterval(unreadDMCountInterval);
    unreadDMCountInterval = setInterval(fetchUnreadDMCount, 30000); // 30秒ごとに更新
};
...
onMounted(() => {
    ...
    startUnreadDMCountInterval();
    // 招待応答時のイベントをリッスン
    window.addEventListener('invitation-count-updated', fetchInvitationCount);
    // 未読DM数即時更新イベントをリッスン
    window.addEventListener('unread-dm-count-updated', fetchUnreadDMCount);
});
...
onUnmounted(() => {
    ...
    window.removeEventListener('unread-dm-count-updated', fetchUnreadDMCount);
});
</script>
